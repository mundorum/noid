...
- **Chain-reaction-safe.** `publish` tracks reentrant-call depth via a contextvar. The outermost handler dispatch in a call chain is awaited directly (so synchronous side effects like `OidBase.set_ready(False)` land immediately). Any dispatch reentered while still inside an outer `publish()` call — e.g. a subscriber that republishes on receipt, as CSV-style row-by-row sources do — is instead wrapped in `asyncio.create_task()` and awaited. That converts the recursive call chain into a sequence of tasks driven by the event loop's ready queue, so chain length no longer costs Python stack depth. See decision #22.
...
| 22 | `Bus.publish` breaks reentrant chains via `asyncio.create_task`, gated by a contextvar depth counter | A subscriber that republishes on receipt (e.g. a CSV source re-requesting the next row every time the previous one is displayed) causes `Bus.publish` to call itself reentrantly for every row. Directly awaiting a bare coroutine chains Python stack frames one per hop (unlike JS, where every `await` unconditionally defers to the microtask queue), so long chains hit `RecursionError` even though `publish()` is `async`. Fix: only the outermost dispatch per call chain is awaited directly (preserves synchronous side effects like `set_ready(False)`, which the readiness queue depends on); any dispatch reentered while already inside a `publish()` call is wrapped in `asyncio.create_task()` instead, which genuinely suspends through the event loop rather than nesting the call stack. |
